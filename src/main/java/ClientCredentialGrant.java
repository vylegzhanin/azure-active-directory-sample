...
class ClientCredentialGrant {
    ...
    private static void setUpSampleData() throws IOException {
        ...
        tenantId = getProperty(properties, "TENANT_ID");
        clientId = getProperty(properties, "CLIENT_ID");
        secret = getProperty(properties, "SECRET");
        scope = getProperty(properties, "SCOPE");
        authority = String.format(getProperty(properties, "AUTHORITY"), tenantId);
    }

    private static String getProperty(Properties properties, String name) {
        String result = properties.getProperty(name, null);
        if (result == null || result.isEmpty()) result = System.getenv(name);
        if (result == null || result.isEmpty()) throw new NullPointerException(name + " property required");
        return result;
    }
}
